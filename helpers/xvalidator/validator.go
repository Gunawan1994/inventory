...
func (v *Validator) StructError(errors map[string]string) error {
...
func (v *Validator) formatValidationError(err error) map[string]string {
	...
	for _, err := range err.(validator.ValidationErrors) {
		...
		case "eq":
			errors[err.Field()] = fmt.Sprintf("%s must be equal to %s", err.Field(), err.Param())
		case "max":
		...
		case "uuid":
			errors[err.Field()] = fmt.Sprintf("%s is not a valid uuid", err.Field())
		default:
			errors[err.Field()] = fmt.Sprintf("%s is not valid. Full error: %s", err.Field(), err.Error())
		}
	}
	return errors
}
